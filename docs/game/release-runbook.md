...
- `release-manifest.json`：产品版本、技术版本、源提交、工作区状态、大小与哈希；
- 本手册和中英双语软件许可。
...
它要求交付目录只包含约定的六个文件，并重新核对四个载荷的产品/技术版本、来源提交、源码树
...
关闭游戏后运行 `pnpm release:verify:installed`。它必须找到唯一的技术版本 1.1.0 卸载项、
有效安装目录和主 EXE，以及至少一个开始菜单或桌面快捷方式；输出同时记录对外产品版本 1.1
及用户数据是否存在，但
不会读取或删除其中的内容。
